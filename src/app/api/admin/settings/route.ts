...
export async function GET(request: Request) {
  ...
  const { rows } = await query('SELECT * FROM restaurant_settings ORDER BY updated_at DESC LIMIT 1');
...
export async function PATCH(request: Request) {
  ...
  const { rows } = await query(
    ...
    [body.name.trim(), body.logo_url || null, body.primary_color || null, body.id]
  );
  return rows[0]
