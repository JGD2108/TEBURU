...
import { jsonAuthorizationError, jsonError, jsonSuccess, readJsonObject, requestId } from '@/lib/api-response';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (signed.error || !signed.data?.token) throw signed.error ?? new Error('Signed upload token was not created');
    ...
    return jsonSuccess(request, { authorization: {
      id,
      objectPath: path,
      ...
      maxBytes: PDF_MENU_MAX_BYTES,
      contentType: 'application/pdf',
    } }, { status: 201 });
  } catch (error) {
    const databaseFailure = menuImportDatabaseFailure(error);
    logger.error('menu_import.upload_authorization_failed', error, { requestId: correlationId, restaurantId, databaseCode: databaseFailure?.databaseCode });
